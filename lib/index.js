...
async function obfuscateBundle(
  ...
  const filesSrc = [`${SRC_TEMP_FOLDER}/**/*.js?(.map)`];
  ...
  await obfuscate({ config, filesSrc, filesDest, cwd });
...
module.exports = function (_config = {}, runConfig = {}, projectRoot = process.cwd()) {
  ...
  // see: https://service.javascriptobfuscator.com/httpapi.asmx?op=JSOExecute
  // and: https://javascriptobfuscator.com/docs/
  const config = Object.assign({}, {
    Items: [],
    APIKey: undefined,
    APIPwd: undefined,
    Name: 'javascript-obfuscator-metro-plugin',
    KeepComment: false,
    EncodeStrings: false,
    MoveStrings: true,
    ReplaceNames: true,
