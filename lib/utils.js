...
/**
 * Detect whether the current process was launched by Expo (`npx expo export`).
 * Expo does not pass `bundle` as a positional arg; instead the argv contains
 * 'export' and the process is typically spawned from within the expo-cli binary.
 * @returns {boolean}
 */
function isExpoExport() {
  const argv = process.argv.join(' ');
  // Matches: `expo export`, `expo export:embed`, invocations via @expo/cli
  return /expo[^\s]*\s+export/.test(argv) || process.argv.includes(EXPO_BUNDLE_CMD);
}

/**
 * Only 'bundle' (bare RN) or 'export' (Expo) commands trigger obfuscation.
 * Development bundles will be ignored (--dev true). Use JSO_METRO_DEV to override.
 * @param {{ runInDev: boolean }} options
 * @returns {string|null} skip reason string, or null to proceed with obfuscation
 */
function skipObfuscation({ runInDev }) {
  const expo = isExpoExport();

  if (!expo) {
    // Bare RN path: require explicit `bundle` positional command
  ...
  // Expo export never passes --dev; treat it as a production build.
  return null;
}

/**
 * Scan Expo output directories for the built JS bundle file.
 * Expo (classic and modern) writes bundles to predictable locations under the
 * project root.  We walk EXPO_OUTPUT_DIRS in priority order and return the
 * first *.js bundle we find (excluding source-map files).
 *
 * @param {string} [projectRoot=process.cwd()]
 * @returns {{ bundlePath: string, bundleSourceMapPath: string|undefined } | null}
 */
function findExpoBundlePath(projectRoot = process.cwd()) {
  for (const dir of EXPO_OUTPUT_DIRS) {
    const absDir = path.resolve(projectRoot, dir);
    if (!fs.existsSync(absDir)) continue;

    // Walk one level deep; Expo may nest per-platform dirs (ios/ android/)
    const entries = fs.readdirSync(absDir, { withFileTypes: true });

    // Check current dir first, then immediate sub-dirs
    const dirsToCheck = [
      absDir,
      ...entries.filter(e => e.isDirectory()).map(e => path.join(absDir, e.name)),
    ];

    for (const checkDir of dirsToCheck) {
      let files;
      try {
        files = fs.readdirSync(checkDir);
      } catch (_) {
        continue;
      }
      const bundle = files.find(f => f.endsWith('.js') && !f.endsWith('.map'));
      if (bundle) {
        const bundlePath = path.join(checkDir, bundle);
        const mapCandidate = path.join(checkDir, bundle + '.map');
        return {
          bundlePath,
          bundleSourceMapPath: fs.existsSync(mapCandidate) ? mapCandidate : undefined,
        };
      }
    }
  }
...
/**
 * Resolve bundle output path using, in priority order:
 *   1. Environment variables  (JSO_BUNDLE_OUTPUT / JSO_SOURCEMAP_OUTPUT)
 *   2. CLI args               (--bundle-output / --sourcemap-output)  – bare RN
 *   3. Expo output-dir scan   (dist/_expo/static/js, dist/bundles, dist)
 *
 * @param {string} [projectRoot=process.cwd()]
 * @returns {{ bundlePath: string, bundleSourceMapPath: string|undefined }}
 */
function getBundlePath(projectRoot = process.cwd()) {
  // 1. Explicit env-var override (works for any toolchain)
  if (process.env[ENV_BUNDLE_OUTPUT]) {
    return {
      bundlePath: process.env[ENV_BUNDLE_OUTPUT],
      bundleSourceMapPath: process.env[ENV_SOURCEMAP_OUTPUT],
    };
  }

  // 2. Bare React Native: --bundle-output CLI arg
  const command = new Command();
  command
  ...
  // 3. Expo: scan well-known output directories
  const expoResult = findExpoBundlePath(projectRoot);
  if (expoResult) {
    return expoResult;
  }
  ...
  console.error(
    ...
    'For Expo projects make sure `npx expo export` has completed before this ' +
    'runs, or set the JSO_BUNDLE_OUTPUT environment variable explicitly.'
  );
